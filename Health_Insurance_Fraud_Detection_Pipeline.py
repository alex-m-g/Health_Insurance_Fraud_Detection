...
def ensemble_model(x_train, y_train, x_test, y_test, x_val, y_val):
    ...
    y_pred_prob = np.mean([y_pred_prob1, y_pred_prob2, y_pred_prob3, y_pred_prob4], axis=0)
...
def data_wrangle(folder):
    ...
    df_test_inpatient_data = pd.read_csv(files[3])
    df_test_outpatient_data = pd.read_csv(files[4])
    df_train_inpatient_data = pd.read_csv(files[5])
    df_train_beneficiary_data = pd.read_csv(files[6])
    df_test = pd.read_csv(files[7])
